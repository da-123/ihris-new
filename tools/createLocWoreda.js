...
function processJobs() {
  if ( !started && !waiting ) {
    started = true
//console.log(lookup)
    fs.createReadStream( input + "woreda.csv" ).pipe( csv() )
      .on( 'data', ( data ) => {
        if ( lookup[data.zone] ) {
          let entry = JSON.parse( JSON.stringify( entryTemplate ) )
          //console.log(lookup[data.region] +" "+data.region)
          let code = lookup[data.zone] + "." + data.code
          entry.resource.id = code
          entry.resource.name = data.name
          entry.resource.partOf.reference += lookup[data.zone]
          entry.request.url += code
          bundle.entry.push( entry )
        } else {
          console.error("Skipping data outside Ethiopia: "+data.zone)
        }
