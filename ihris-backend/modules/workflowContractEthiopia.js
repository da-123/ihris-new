...
const workflowContractEthiopia = {
  process: ( req ) => {
      ...
      try {
        ...
        if (resource.resourceType === "Practitioner") {
          if ( req.body && req.body.item 
            ...
            resolve(await workflowContractEthiopia.outcome("Either Duty Post or Job title not provided"))
          }
        } else {
          winston.error("No Person(Practitioner) Record found")
          resolve(await workflowContractEthiopia.outcome("No Person(Practitioner) Record found"))
        }
      } catch(err) {
        reject(err)
      } 
...
module.exports = workflowContractEthiopia
